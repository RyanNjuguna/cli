...
mealplan_app = typer.Typer()

@mealplan_app.command("add")
def add_plan(user: str, week: int, description: str):
    ...
    plan = MealPlan(user_id=user_obj.id, week=week, description=description, created_at=datetime.utcnow())
    session.add(plan)
    session.commit()
    typer.echo(f"Meal plan added for week {week}.")

@mealplan_app.command("update")
